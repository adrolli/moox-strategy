# My Strategy for moox

This is the strategy for moox. 

This repo contains roadmaps (in German language) and specs (in English language) that describe, how moox should evolve in the next months. 

## Ordered overview

- AI Foundation
- [Foundation](Foundation.md) - Cleanup Core, encapsulate BaseClasses, Prepare Media for translated images, Attributes vs. Custom Types and Fields
  - ...
- AI Iteration 1
- [Press](Press.md) - moox Headless (API), moox Sync (rewrite), moox Press (multilang, synced)
  - [Iteration 0](Press.Spec0.md)
- AI Iteration 2
- [Frontend](Frontend.md) - Routing, Themes, Components ...
- AI Iteration 3
- Installation, Updates
- AI Finalizing
